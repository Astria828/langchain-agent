...
from app.db.models import BackgroundTask, LongTermMemory, ModelConfig
...
def test_message_api_rejects_required_index_rebuild_before_saving_user(
    ...
    try:
        ...
        assert client.get(f"/api/sessions/{created['id']}/messages").json()["data"] == []
    finally:
        client.close()
        engine.dispose()
...
def test_memory_api_filters_orders_and_invalidates_idempotently(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert [memory["id"] for memory in invalid_memories] == [
            "mem_role_one_invalid"
        ]
        ...
        assert (
            client.post("/api/memories/missing/invalidate", json={}).status_code
            == 404
        )
...
def test_memory_invalidation_keeps_sqlite_state_when_vector_delete_fails(
    ...
    try:
        ...
        with session_factory() as session:
            ...
            cleanup_tasks = session.query(BackgroundTask).filter_by(
                task_type="vector_cleanup",
                scope_id="mem_cleanup_required",
            ).all()
            assert len(cleanup_tasks) == 1
            assert cleanup_tasks[0].status == "pending"
